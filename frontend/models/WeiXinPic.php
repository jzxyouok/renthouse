...
class WeiXinPic extends ActiveRecord
{
    ...
    {
        Yii::error("WeiXinPic::AddWxPic jsonObj is ".print_r($jsonObj,1)."\n"); 
        try{
            //1.插入数据
                $reqlist=WeiXinPic::getWeiXinPic($jsonObj->serverid);
                if(!empty($reqlist)){
                        Yii::error("already exists\n");
                        $arr['rscode']=1;
                        $arr['error']='记录已经存在';
                        return $arr;
                }
            ...
            //2.根据serverid获取图片并保存到本地服务器
                ...
                switch ($jsonObj->type){
                    case 1:
                        $directory = "/path/to/rent/frontend/web/images/head/";
                        $fullname = "http://www.lovespace.cc/images/head/".$rename;
                        break;
                    case 2:
                        $directory = "/path/to/rent/frontend/web/images/house/";
                        $fullname = "http://www.lovespace.cc/images/house/".$rename;
                        break;
                    case 3:
                        $directory = "/path/to/rent/frontend/web/images/report/";
                        $fullname = "http://www.lovespace.cc/images/report/".$rename;
                        break;
                    default:
                        $arr['rscode']=1;
