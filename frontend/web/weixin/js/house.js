(function(h) {
	...
	//创建房源详情slidergroup
	h.fillHouseDetailSliderGroupNew = function(data) {
		var ele = document.createElement("div");
		ele.className = "mui-slider-group mui-slider-loop";
		...
		for(var i in data)
		{
			...
			if(i == 0)
			{
				liContent += '<div style="background-image:url('+data[i].picture+');" class="mui-slider-item mui-slider-item-duplicate" promotion-id="'+data[i].id+'">';
				liContent += '<img src="'+data[i].picture+'">';
				liContent += '</div>';
				liContent += '<div style="background-image:url('+data[i].picture+');" class="mui-slider-item" promotion-id="'+data[i].id+'">';
				liContent += '<img src="'+data[i].picture+'">';
				liContent += '</div>';
			}
			else if(i == data.length-1)
